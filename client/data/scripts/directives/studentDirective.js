(function()
{
    ...
    app.directive("studentDirective", function(studentHomeService, userService)
    {
        function getSchedule(param)
        {
            ...
            var promiseObj = studentHomeService.getScheduleForStudent(user);
                ...
                element.on("mouseenter", `[data-info="true"]`, function(event)
                {
                    ...
                    if (elem.attr("id") !== undefined && elem[0].className == "active-date")
                    {
                        ...
                        for (var item of dayInfo)
                        {
                            ...
                            if (counter > 0)
                            {
                                elem.children().append(`<div data-id="${id}" class="info-block" style="margin-bottom: 10%;"><div class="info-text">${info}</div></div>`);
                            }
                            else
                            {
